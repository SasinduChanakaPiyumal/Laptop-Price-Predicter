...
def add_company(company_name, company_mapping=None):
    """
    Categorize companies by mapping less common ones to 'Other'.
    
    This function takes a company name and returns either the original name
    or 'Other' if the company is in the mapping list. The mapping can be
    provided explicitly or loaded from the configuration file.
    
    Parameters:
    -----------
    company_name : str
        The name of the company to categorize
    company_mapping : list/set, optional
        A collection of company names to map to 'Other'.
        If None (default), the mapping is loaded from the config file
        using load_config()['company_mapping'].
    
    Returns:
    --------
    str
        Either 'Other' if the company is in the mapping, or the original
        company_name if it's not.
    
    Examples:
    ---------
    >>> add_company('Samsung')  # Loads from config
    'Other'
    >>> add_company('Dell')  # Loads from config
    'Dell'
    >>> add_company('Samsung', ['Samsung', 'LG'])  # Uses provided mapping
    'Other'
    """
    if company_mapping is None:
        config = load_config()
        company_mapping = config['company_mapping']
