...
def load_config(config_path='model_config.json'):
    """
    Load configuration file containing categorization mappings for data preprocessing.
    
    This function reads a JSON configuration file that contains mappings for 
    categorizing companies and operating systems. It validates that all required
    keys are present in the configuration.
    
    Parameters:
    -----------
    config_path : str, optional (default='model_config.json')
        Path to the JSON configuration file. Can be relative or absolute path.
        
    Returns:
    --------
    dict
        Dictionary containing configuration mappings with the following required keys:
        - 'company_mapping': Mapping of company names to categories
        - 'os_mapping': Mapping of operating system names to categories
    ...
    KeyError
        If required keys ('company_mapping' or 'os_mapping') are missing from the config.
        
    Examples:
    ---------
    >>> config = load_config()  # Uses default path
    >>> config = load_config('custom_config.json')  # Uses custom path
    >>> company_map = config['company_mapping']
    >>> os_map = config['os_mapping']
    """
    try:
        ...
        raise FileNotFoundError(
            ...
            f"Please ensure the config file exists in the expected location."
        )
    except json.JSONDecodeError as e:
        raise json.JSONDecodeError(
            f"Configuration file '{config_path}' contains malformed JSON. "
            f"Please check the file syntax. Error: {e.msg}",
            e.doc,
            e.pos
        )
    
    # Validate required keys
    required_keys = ['company_mapping', 'os_mapping']
    for key in required_keys:
        if key not in config:
            raise KeyError(
                f"Required key '{key}' is missing from configuration file '{config_path}'. "
