...
def load_data(file_path, encoding='latin-1'):
    """
    Load a CSV file into a pandas DataFrame with configurable encoding.
    ...
    encoding : str, optional
        Character encoding to use for reading the file. Default is 'latin-1'.
        Common alternatives include 'utf-8', 'iso-8859-1', 'cp1252'.
    ...
    ------
    TypeError
        If the encoding parameter is not a string.
    FileNotFoundError
        If the specified file does not exist at the given path.
    UnicodeDecodeError
        If the file cannot be decoded with the specified encoding.
        Try alternative encodings like 'utf-8' or 'iso-8859-1'.
    
    Examples
    --------
    >>> df = load_data("laptop_price.csv")
    >>> df = load_data("data.csv", encoding='utf-8')
    """
    # Validate encoding parameter type
    if not isinstance(encoding, str):
        raise TypeError(
            f"encoding parameter must be a string, got {type(encoding).__name__}"
        )
    ...
    try:
        ...
        raise UnicodeDecodeError(
            ...
            f"Try alternative encodings like 'utf-8', 'iso-8859-1', or 'cp1252'."
        )
