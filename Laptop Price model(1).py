...
# OPTIMIZED: Extract all values at once instead of calling function 3 times per row
def extract_resolution(res_string):
    ...
    if match:
        ...
        return pd.Series([width, height, width * height])  # width, height, total pixels
    return pd.Series([1366, 768, 1366*768])  # default resolution if not found

# Apply once and unpack all three values efficiently
...
def extract_storage_features(memory_string):
    ...
    OPTIMIZED: Returns pandas Series for efficient unpacking
    """
...
# OPTIMIZED: Apply once and unpack all five values efficiently
dataset[['Has_SSD', 'Has_HDD', 'Has_Flash', 'Has_Hybrid', 'Storage_Capacity_GB']] = dataset['Memory'].apply(extract_storage_features)
